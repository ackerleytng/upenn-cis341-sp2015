(* CIS341 HW1 *)
(* hellocaml.ml *)
(* This is an individual project -- please complete all of the work yourself *)
 ...
 * (You may also want to consider using 'utop' a more featureful toplevel loop.)
 ...
 * #
 *
 * # is the prompt  (in Eclipse commands are entered into the bottom of the window)
 *
 ...
 * # #quit;;
 *
 * In Eclipse, you can reset the top-level loop by clicking on the yellow "arrows" icon
 * at the top right of the interaction window.
 *
 ...
 * "case3" passes, given the definition below:
 *)
let prob3_case3 : int =
...
(*
 * In this an later projects, you can add your own test cases to the
 ...
 *)
let third_of_three (t:'a * 'b * 'c) : 'c = match t with
  | (_, _, x) -> x
 ...
 *)
let rec list_to_mylist (l:'a list) : 'a mylist = match l with
  [] -> Nil
...
(* Want to quickly get this over with, will be programming with Core anyway *)
 ...
 *
 * Tail recursion is important to efficiency -- OCaml will compile
...
let rec insert (x:'a) (l:'a list) : 'a list =
  if List.exists l (fun e -> e = x) then l
  else
    let less_than, more_than = List.partition_tf l (fun e -> (compare e x) < 0) in
    less_than @ x :: more_than                                  
...
let rec interpret (c:ctxt) (e:exp) : int32 =
  ...
  | Var s -> lookup s c
  | Const x -> x
  | Add (a, b) -> Int32.(+) (interpret c a) (interpret c b)
  | Mult (a, b) -> Int32.( * ) (interpret c a) (interpret c b)
...
let rec optimize (e:exp) : exp =
  match e with 
  | Add (Const a, Const 0l) | Add (Const 0l, Const a) -> Const a
  | Add (Const a, Const b) -> Const (Int32.(+) a b)
  | Add (a, b) -> optimize (Add ((optimize a), (optimize b)))
  | Mult (Const 0l, _) | Mult (_, Const 0l) -> Const 0l
  | Mult (Const 1l, a) | Mult (a, Const 1l) -> optimize a
  | Mult (Const a, Const b) -> Const (Int32.( * ) a b)
  | Mult (Const a, Neg (Const b)) | Mult (Neg (Const a), Const b)
    -> optimize (Neg (optimize (Mult (Const a, Const b))))
  | Mult (a, b) -> optimize (Mult ((optimize a), (optimize b)))
  | Neg (Const 0l) -> Const 0l
  | Neg (Neg v) -> v
  | Neg (a) -> Neg (optimize a)
  | Var _ | Const _ as ee -> ee
...
let step (c:ctxt) (s:stack) (i:insn) : stack =
  begin match (i, s) with
    ...
    | (IMul, v1::v2::s) -> (Int32.(v1 * v2))::s
    | (IAdd, v1::v2::s) -> (Int32.(v1 + v2))::s
 ...
 *)
let execute' (c:ctxt) (p:program) : stack =
  List.fold_left ~init:[] ~f:(fun s i -> step c s i) p
...
let p2 = [IPushV "x"; IPushC 3l; IMul]
let context2 = ["x", 3l]
 ...
 * quite far.  
 *
 * Take a quick read through util/assert.ml -- this is a small libary for 
